...
Based on the intersection rule we found 2 parameter candidates for low volatility regime, for the final Out-of-Sample test we choose the parameter set *lbk=40_band=2.5* as it outperforms *lbk=20_band=2* on *mean_return* and *10_percentile_return* metrics.
...
On the high volatility regime we just found 1 parameter set that had a positive return in the backtests i.e *lbk=10_band=1.5*, even though the return profile seems insignificant it could be down to short length of the high volatility prices. For the sake of completeness we will include this parameter in the final strategy as well. 
...
```
strat_signals_act = gen_bband_multi_regime_signals(act_os_vix_df, 10, 1.5, 10, 1.5, 25)
underlying_returns = act_os_vix_df["Adj_Close"].pct_change(1)
strat_perf_act = strat_signals_act.shift(2)*underlying_returns
regime_act_df = pd.DataFrame({"params": ["lbk=10/10_band=1.5/1.5"], "mean_return":strat_perf_act.sum(axis=0).mean(), "stdev_return":strat_perf_act.sum(axis=0).std(), "10_percentile_return": np.percentile(strat_perf_act.sum(axis=0), 0.1)})
regime_act_df
===========================================================================
idx         params	    mean_return stdev_return 10_percentile_return
0   lbk=10_/10_band=1.5/1.5	0.050112    NaN	         0.050112
```
...
strat_perf_act_df = act_os_vix_df.join(strat_perf_df, how="left").fillna(0)
strat_perf_act_df["VIX Close Lag"] = strat_perf_act_df["VIX Close"].shift(1).fillna(0)
strat_act_reg_1 = strat_perf_act_df[strat_perf_act_df["VIX Close Lag"]<=25]
strat_act_reg_2 = strat_perf_act_df[strat_perf_act_df["VIX Close Lag"]>25]
...
========================================================================================
"strategy return in low volatility regime is: -0.014224233848727286 and in high volatility regime is: 0.0643364071642365"
...
```
strat_signals_syn = gen_bband_multi_regime_signals(act_os_vix_df, 40, 2.5, 10, 1.5, 25)
strat_perf_syn = strat_signals_syn.shift(2)*underlying_returns
regime_syn_df = pd.DataFrame({"params": ["lbk=40/10_band=2.5/1.5"], "mean_return":strat_perf_syn.sum(axis=0).mean(), "stdev_return":strat_perf_syn.sum(axis=0).std(), "10_percentile_return": np.percentile(strat_perf_syn.sum(axis=0), 0.1)})
regime_syn_df
...
strat_syn_reg_1 = strat_perf_syn_df[strat_perf_syn_df["VIX Close Lag"]<=25]
strat_syn_reg_2 = strat_perf_syn_df[strat_perf_syn_df["VIX Close Lag"]>25]
...
=============================================================================
"strategy return in low volatility regime is: 0.09899699273763563 and in high volatility regime is: 0.0643364071642365"
